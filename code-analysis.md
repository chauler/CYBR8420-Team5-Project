...
### Findings Related to RequestServer Network Isolation
My review of Ladybird’s RequestServer-based networking path identified several potential weaknesses mapped to CWE-285, CWE-441, CWE-923, and CWE-770. Centralizing all network activity in a dedicated RequestServer process is a good architectural choice, but the current documentation and code paths do not clearly show strict authorization decisions for which browser components may perform which network actions, raising CWE-285 concerns. Because RequestServer forwards outbound traffic on behalf of less-privileged WebContent processes, it can act as a confused deputy (CWE-441) if it is not carefully constrained from becoming a generic proxy to internal or otherwise restricted endpoints. The IPC and connection plumbing also rely on multiple communication channels, and the binding of these channels to their intended endpoints is not visibly enforced at a design level, leaving room for CWE-923-style misrouting or spoofing risks. Also for CWE-770, there was an open issue #3185 that can be mitigated by adding stronger resource limits and backpressure inside RequestServer so a single heavy or malicious page can’t exhaust file descriptors. This includes per-client and global caps on concurrent requests, limited connection/pipe pooling, and signals back to WebContent to slow or defer new loads when RequestServer is saturated. These changes both fix the “too many open files” problem and strengthen the RequestServer isolation claim by keeping the network broker a controlled, resilient choke point between untrusted renderers and the external network.
